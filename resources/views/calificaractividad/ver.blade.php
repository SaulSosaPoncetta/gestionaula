...
@section('content')
    ...
    <div class="row mb-4">
        ...
        <div class="col-auto d-flex gap-2">
            <a href="{{ route('calificaractividad.incompletas', ['materia_id' => $materia->id, 'curso_id' => $curso->id]) }}"
                class="btn btn-outline-warning">
                <i class="bi bi-exclamation-triangle me-1"></i>Incompletas
            </a>
            <a href="{{ route('calificaractividad.calificadas', ['materia_id' => $materia->id, 'curso_id' => $curso->id]) }}"
                class="btn btn-outline-success">
                <i class="bi bi-check2-circle me-1"></i>Calificadas
            </a>
                            ...
                            <tbody>
                                @foreach ($actividades as $act)
                                    @php
                                        ...
                                        $reg = $estadosRegistrados->get($key);
                                        // Saltar si ya está calificada
                                        if (in_array($key, $yaCalificadas)) {
                                            continue;
                                        }
                                    @endphp
                                    <tr>
                                        ...
                                        <td>
                                            <select name="registros[{{ $alumno->id }}][{{ $act->id }}][estado]"
                                                ...
                                                data-alumno="{{ $alumno->id }}" data-actividad="{{ $act->id }}">
                                        ...
                                        <td>
                                            <input type="date" class="form-control form-control-sm campo-fecha"
                                        ...
                                        <td>
                                            <input type="number" class="form-control form-control-sm"
                                                ...
                                                value="{{ $reg?->nota ?? '' }}" min="0" max="10"
                                                step="0.25" placeholder="—">
                                        </td>
                                        <td>
                                            <input type="text" class="form-control form-control-sm"
                                                name="registros[{{ $alumno->id }}][{{ $act->id }}][observacion]"
                                                value="{{ $reg?->observacion ?? '' }}" placeholder="Opcional">
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
