...
<div class="row mb-4">
    <div class="col">
        ...
        <p class="text-muted">Los ítems se pre-cargan desde tus horarios registrados. Podés editarlos antes de guardar.</p>
    </div>
    <div class="col-auto">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-3">
                    ...
                    <input type="date" name="fechadeclaracion"
                           ...
                           value="{{ old('fechadeclaracion', date('Y-m-d')) }}" required>
...
const cursosData       = @json(\App\Models\Curso::orderBy('anio')->orderBy('division')->get()->map(fn($c) => ['id' => $c->id, 'nombre' => $c->nombre_completo]));
const materiasData     = @json(\App\Models\Materia::orderBy('nombre')->get()->map(fn($m) => ['id' => $m->id, 'nombre' => $m->nombre]));
...
function agregarFila() {
    ...
    tr.innerHTML = `
        ...
        <td>
            <select name="items[${indice}][curso_id]" class="form-select form-select-sm selectcurso" data-selected=""></select>
        </td>
        <td>
            <select name="items[${indice}][materia_id]" class="form-select form-select-sm selectmateria" data-selected=""></select>
        </td>
        <td class="text-center">
            <button type="button" class="btn btn-sm btn-outline-danger eliminarfila">
                <i class="bi bi-trash"></i>
...
// Inicializar selects en filas existentes
document.querySelectorAll('.fila-horario').forEach(fila => {
    const selectCurso   = fila.querySelector('.selectcurso');
    const selectMateria = fila.querySelector('.selectmateria');
    if (selectCurso)   llenarSelectCursos(selectCurso, selectCurso.dataset.selected);
    if (selectMateria) llenarSelectMaterias(selectMateria, selectMateria.dataset.selected);
});
